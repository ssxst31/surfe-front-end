              ...
              <ul>
                {friendList.map((friend) => (
                  <li key={friend.id} className="relative flex justify-between p-3 rounded-md hover:bg-gray-100">
                    <div className="flex">
                      <img src={friend.profile} className="rounded-[50%] h-14 w-14" alt="profile" />
                      {friend.nickname}
                    </div>
                    <button className="w-20 px-4 py-2 text-xs font-medium text-white bg-blue-500 rounded-lg hover:bg-blue-600">
                      삭제
                    </button>
                  </li>
                ))}
